...
const bsOffCanvasInit = () => {
  ...
  if (offcanvasElement) {
    ...
    offcanvasElement.addEventListener("hidden.bs.offcanvas", function () {
      ...
      // Restore scroll position smoothly after offcanvas closes
      setTimeout(() => {
        window.scrollTo({
          top: scrollPosition,
          behavior: 'smooth'
    ...
    // Prevent scroll restoration on hide event
    offcanvasElement.addEventListener("hide.bs.offcanvas", function () {
      // Update scroll position right before hiding
      scrollPosition = window.pageYOffset || document.documentElement.scrollTop;
    });
  }
};
  ...
  scrollLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      if (href.startsWith('#')) {
        ...
        // Close the offcanvas first (mobile menu)
        if (offcanvasElement) {
        ...
        }, 300); // Wait for offcanvas closing animation
      }
      // If it's a regular link (like index.html or site.html), let it navigate normally
    });
  });
};
