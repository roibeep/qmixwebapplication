...
interface Project { projectID: number; name: string; project_location: string; end_location?: string | null; design_mix?: string | null; customer?: Customer | null; }
...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Manage Projects and Track Deliveries', href: '/superadmin/projects' }];

export default function ProjectIndex() {
  const { projects, clients, auth } = usePage<{ projects: Project[]; clients: Customer[]; auth: any }>().props;

  const currentUser = auth.user;
  const isAuthorized = currentUser.role?.toLowerCase() === 'superadmin' || currentUser.department?.name?.toLowerCase() === 'production';

  /* ================= STATE ================= */
  const [search, setSearch] = useState('');
  const [openProjectDialog, setOpenProjectDialog] = useState(false);
  const [editProjectId, setEditProjectId] = useState<number | null>(null);
  const [projectForm, setProjectForm] = useState({ customerID: '' as number | '', name: '', project_location: '', end_location: '', design_mix: '' });
  ...
  const submitProject = (e: React.FormEvent) => {
    e.preventDefault();
    if (editProjectId) {
      router.put(`/superadmin/projects/${editProjectId}`, projectForm, {
        onSuccess: () => router.reload({ only: ['projects'] }),
      });
    } else {
      router.post('/superadmin/projects/store', projectForm, {
        onSuccess: () => router.reload({ only: ['projects'] }),
      });
    }
    closeProjectDialog();
  ...
  const deleteProject = (id: number) => {
    ...
    router.delete(`/superadmin/projects/${id}`, {
      onSuccess: () => router.reload({ only: ['projects'] }),
    });
  };
  ...
  const openDeliveryList = async (proj: Project) => {
    ...
    try {
      const res = await fetch(`/superadmin/projects/${proj.projectID}/deliveries`);
  ...
  const submitDelivery = (e: React.FormEvent) => {
    ...
    const url = editDeliveryId
      ...
      : `/superadmin/projects/${selectedProject.projectID}/deliveries`;
    ...
    router[method](url, editDeliveryId ? { ...deliveryForm, projectID: selectedProject.projectID } : deliveryForm, {
      onSuccess: async () => {
        const res = await fetch(`/superadmin/projects/${selectedProject.projectID}/deliveries`);
        setDeliveryList(await res.json());
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      {/* ======= Add/Edit Delivery Dialog (Updated with Status Dropdown) ======= */}
